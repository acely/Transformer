...
import cn.acely.ui.XScrollPane;
import cn.acely.ui.XSplitPanUI;
...
public class Editor {
	...
	private JButton copyDefaultComment = new JButton(Editor.labels.getString("copy_comment"));
	...
	private JButton copyDefaultText = new JButton(Editor.labels.getString("copy_text"));
...
//		frame.setLocation(x,y);
		...
		translationArea.setForeground(Color.WHITE);
		...
		copyDefaultText.setFocusable(false);
		copyDefaultComment.setFocusable(false);
		...
		commentLabelPanel.add(copyDefaultComment,BorderLayout.EAST);
		commentPane.add(commentLabelPanel);
		commentPane.add(commentArea);

		useDefaultComment.setFocusable(false);
		useDefaultComment.addItemListener(new ItemListener() {
			@Override
		...
		copyDefaultComment.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				if (!modifying){
					modifying = true;
					commentArea.setText(getDefaultComment());
					modifying = false;
				}
			}
		});
		...
		translationLabelPanel.add(copyDefaultText,BorderLayout.EAST);
		translationPane.add(translationLabelPanel);
		translationPane.add(translationArea);

		useDefaultText.setFocusable(false);
		useDefaultText.addItemListener(new ItemListener() {
			@Override
			public void itemStateChanged(ItemEvent e) {
				if (!modifying){
					System.out.println("useDefaultText.stateChanged");
					modifying = true;
					boolean isTop = (workingNode == null || top == workingNode);
					if (useDefaultText.isSelected()) {
						translationArea.setText("");
						commentArea.setText("");
						useDefaultComment.setEnabled(false);
						useDefaultComment.setSelected(!isTop);
						commentArea.setEditable(false);
					}else {
						useDefaultComment.setEnabled(!isTop);
						useDefaultComment.setSelected(!isTop);
		...
		copyDefaultText.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				if (!modifying){
					modifying = true;
					translationArea.setText(getDefaultProperty());
					modifying = false;;
				}
			}
		});
		...
		textSplitPane = new JSplitPane(
			...
			new XScrollPane(translationPane),
			new XScrollPane(commentPane)
		);
		textSplitPane.setUI(new XSplitPanUI());
		textSplitPane.setBorder(null);
		...
		tree.setCellRenderer(new DefaultTreeCellRenderer(){
			...
			public Component getTreeCellRendererComponent(
				...
				setForeground(modified?Color.CYAN:Color.WHITE);
				return this;
			}
		});
		...
		table.setDefaultRenderer(Object.class, new DefaultTableCellRenderer(){
			...
			public Component getTableCellRendererComponent(
				...
				if (name.isDefault) color = Color.ORANGE;
				if (name.isModified) color = Color.CYAN;
				setForeground(color);
				return this;
			}
		});
		...
		navigationSplitPane = new JSplitPane(
			JSplitPane.VERTICAL_SPLIT,
			new XScrollPane(tree),
			new XScrollPane(table)
		);
		navigationSplitPane.setUI(new XSplitPanUI());
		navigationSplitPane.setBorder(null);
		...
		editingSplitPane.setUI(new XSplitPanUI());
		editingSplitPane.setBorder(null);
		
		frame.getContentPane().add(editingSplitPane);

		editingSplitPane.setDividerLocation(250);

		textSplitPane.setResizeWeight(0.7);
//		textSplitPane.setDividerLocation(splitpaneLocation);

		navigationSplitPane.setResizeWeight(0.4);
//		navigationSplitPane.setDividerLocation(splitpaneLocation);
		...
		loadMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_O, getCtrlMask()));
		loadMenuItem.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				...
				fileDialog.pack();
				fileDialog.setDirectory(props.getProperty("open_directory", System.getProperty("user.home")));
				fileDialog.setVisible(true);
				if (fileDialog.getFile() != null) {//如果打开了文件
					props.setProperty("open_directory", fileDialog.getDirectory());
					try {
		...
		saveMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S, getCtrlMask()));
		...
		addKeyMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_K, getCtrlMask()));
		...
		renameKeyMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_R, getCtrlMask()));
		...
		addLangMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_L, getCtrlMask()));
		...
		JMenuItem previousKeyMenuItem = new JMenuItem(Editor.labels.getString("previous_key_menu_name"));
		previousKeyMenuItem.setMnemonic(labels.getString("previous_key_menu_key").charAt(0));
		previousKeyMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_OPEN_BRACKET, getCtrlMask()));
		previousKeyMenuItem.getAccessibleContext().setAccessibleDescription(labels.getString("previous_key_menu_description"));
		previousKeyMenuItem.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				if (workingIndex > 0){
					workingIndex--;
		...
		actionsMenu.add(previousKeyMenuItem);
		...
		nextKeyMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_CLOSE_BRACKET, getCtrlMask()));
	...
	private ActionListener addKeyActionListener = new ActionListener(){
		public void actionPerformed(ActionEvent e){
			String inputValue = JOptionPane.showInputDialog(Editor.labels.getString("add_key_message"),workingName);
			TranslationData data = (TranslationData)top.getUserObject();
			if (inputValue != null){
	...
	private ActionListener renameKeyActionListener = new ActionListener(){
		public void actionPerformed(ActionEvent e){
			...
			String inputValue = JOptionPane.showInputDialog(Editor.labels.getString("rename_key_message"),workingName);
			TranslationData data = (TranslationData)top.getUserObject();
			if (inputValue != null){
				if (data.properties.contains(inputValue)){
					JOptionPane.showMessageDialog(
	...
	public static void main(String[] args) throws IOException {
		if (System.getProperty("os.name").toLowerCase().indexOf("mac") != -1) {
	    	...
	    	System.setProperty("com.apple.mrj.application.apple.menu.about.name", "Transformer");
		}
		try {
			UIManager.setLookAndFeel(new DarculaLaf());
		}catch ( UnsupportedLookAndFeelException e ) {
			System.out.println("不支持Darcular视觉样式");
		}
		String load = null;
		if (args.length > 0) load = args[0];
		Editor e = new Editor((load == null)?null:new File(load));
		System.out.println(e.getCtrlMask());
	}
	...
	public static void setCenterOnScreen(Component component) {
		Dimension screen = Toolkit.getDefaultToolkit().getScreenSize();
		component.setLocation((screen.width-component.getWidth())/2,(screen.height-component.getHeight())/2);
	}
}
